...
func insertRequestWithRepo(ctx context.Context, repo Repository, rec Record, ts time.Time, costCents int64, costMicros int64) error {
	latency := rec.Latency.Milliseconds()
	if latency < 0 {
		latency = 0
	}

	_, err := repo.InsertRequestRecord(ctx, db.InsertRequestRecordParams{
		TenantID:       toPgUUID(rec.Context.TenantID),
		ApiKeyID:       toPgNullableUUID(rec.Context.APIKeyID),
		Ts:             pgtype.Timestamptz{Time: ts, Valid: true},
		ModelAlias:     rec.Alias,
...
func insertUsageWithRepo(ctx context.Context, repo Repository, rec Record, ts time.Time, costCents int64, costMicros int64) error {
	_, err := repo.InsertUsageRecord(ctx, db.InsertUsageRecordParams{
		TenantID:      toPgUUID(rec.Context.TenantID),
		ApiKeyID:      toPgNullableUUID(rec.Context.APIKeyID),
		Ts:            pgtype.Timestamptz{Time: ts, Valid: true},
		ModelAlias:    rec.Alias,
